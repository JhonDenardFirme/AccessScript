...
const CodeEditorv2 = () => {
    ...
    const [parsedResult, setParsedResult] = useState(null);  // Store JSON result
    const [formattedJSON, setFormattedJSON] = useState("");  // Store the string result
    const [errors, setErrors] = useState([]);  // Store error array
    ...
    useEffect(() => {
        if (tokens.length > 0) {
            // Run BNF parser once tokens are updated and get the parsed result
            const { parsedResult, formattedJSON, errors } = BNFParser(tokens);
            setParsedResult(parsedResult);
            setFormattedJSON(formattedJSON);
            setErrors(errors);
        }
    }, [tokens]); // Dependency array ensures this runs after tokens state updates
