...
export function parsePushStatement(tokens, errors) {
    // Consume 'push' keyword (type and value must match)
    consumeToken(tokens, "push", "KEYWORD", errors);
    ...
    if (nextToken?.type === "IDENTIFIER") {
        ...
        if (!userToken) {
            return {
                ...
                push: null,
                pushToGroup: null,
                error: "Expected IDENTIFIER inside 'push'"
            };
        }
        ...
        if (!userToken) {
            return {
                ...
                push: null,
                pushToGroup: null,
                error: "Expected EMAILSTR_LITERAL inside 'push'"
            };
        }
        user = userToken.value;  // Assign value to 'user'
    }
    ...
    if (!groupToken) {
        return {
            ...
            push: user,
            pushToGroup: null,
            error: "Expected IDENTIFIER for group inside 'push'"
        };
    }
    ...
    return {
        type: "PushStatement",
        push: user,  // The user will now be assigned correctly
        pushToGroup: group,  // The group will now be assigned correctly
