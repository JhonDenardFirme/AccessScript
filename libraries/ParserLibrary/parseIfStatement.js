...
export function parseIfStatement(tokens, errors) {
    ...
    while (tokens.length > 0) {
        ...
        // Parsing a function definition or the body
            ...
            consumeToken(tokens, "if", "KEYWORD", errors);
            ...
            if (nextToken2?.type === "DOT_OP") {
                consumeToken(tokens, ".", "DOT_OP", errors);
                consumeToken(tokens, null, "IDENTIFIER", errors);
            }

            let nextToken = tokens[0];
            let conditionalToken = null;

            if (nextToken?.type === "RELATIONAL_OP") {
                conditionalToken = consumeToken(tokens, null, "RELATIONAL_OP", errors);
            }

            else if (nextToken?.type === "EQUALITY_OP") {
                conditionalToken = consumeToken(tokens, null, "EQUALITY_OP", errors);
            }
            ...
            if (!conditionalToken){
                return{
                    ...
                    params: paramList,
                    body: [],
                    error: "Conditional Statement is Expected",
                }
            }
            ...
            let nextToken3 = tokens[0];
                
            if (nextToken3?.type === "DOT_OP") {
                consumeToken(tokens, ".", "DOT_OP", errors);
                consumeToken(tokens, null, "IDENTIFIER", errors);
            }
