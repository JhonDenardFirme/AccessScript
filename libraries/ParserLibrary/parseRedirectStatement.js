...
export function parseRedirectStatement(tokens, errors) {
    // Consume 'assign' keyword (type and value must match)
    consumeToken(tokens, "redirect", "KEYWORD", errors);
    ...
    if (!routeToken) {
        errors.push("Expected STRING or IDENTIFIER inside 'redirect'");
        console.warn("Expected STRING or IDENTIFIER inside 'redirect'");
        return {
            type: "RedirectStatement",
            route: null,
            error: "Expected STRING or IDENTIFIER inside 'redirect'"
        };
    }
